...
export default function BudgetPage() {
  ...
  async function handleDeleteItem(itemId: number) {
    if (!confirm('Supprimer ce poste de dépense ? Les dépenses associées seront conservées mais détachées.')) {
      return;
    }
    try {
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-1 overflow-x-auto no-scrollbar">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-xl whitespace-nowrap shrink-0 transition-all ${
            activeTab === 'budget'
        ...
        >
          ...
          <span>Postes de dépenses ({summary?.items.length || 0})</span>
        </button>
      </div>
          ...
          {summary.items.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center text-slate-500 shadow-xs">
              ...
              <p className="text-base font-bold text-slate-800">
                Aucun poste de dépense configuré pour {selectedYear}
              </p>
              <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
                Commencez par ajouter vos postes de dépenses prévisionnelles dans l'onglet Postes de dépenses ou copiez le budget d'une année précédente.
              </p>
              {isManager && (
                <div className="mt-5 flex justify-center gap-3">
                  ...
                  >
                    ...
                    <span>Ajouter un premier poste de dépense</span>
                  </button>
                  <button
                    onClick={() => setShowNewYearModal(true)}
                ...
                <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-slate-50/50">
                  ...
                  {isManager && (
                    <button
                      ...
                      className="flex items-center gap-2 px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs shadow-xs transition-all self-start sm:self-auto"
                    >
                      <Coins className="w-4 h-4" />
            ...
            <div className="md:hidden divide-y divide-slate-200">
              {summary.items.map((item) => {
                const isOver = item.variance > 0;
                return (
                  <div key={item.id} className="p-4 space-y-2.5 hover:bg-slate-50/60 transition-colors">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="text-xl p-1.5 bg-slate-100 rounded-lg shrink-0">{item.icon}</span>
                        <div className="min-w-0">
                          <div className="font-extrabold text-slate-900 text-sm truncate">{item.name}</div>
                          {item.supplier && (
                            <div className="text-[11px] text-slate-500 font-medium truncate">{item.supplier}</div>
                          )}
                        </div>
                      </div>
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-600 capitalize shrink-0">
                        {item.periodicity}
                      </span>
                    </div>

                    <div className="grid grid-cols-3 gap-2 bg-slate-50 p-2 rounded-lg text-xs">
                      <div>
                        <span className="text-[10px] text-slate-400 block font-medium">Prévision</span>
                        <span className="font-mono font-bold text-slate-800 text-xs">{fmt(item.forecast)}</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 block font-medium">Réel</span>
                        <span className="font-mono font-extrabold text-indigo-600 text-xs">{fmt(item.real)}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] text-slate-400 block font-medium">Écart</span>
                        <span
                          className={`inline-block text-[11px] font-mono font-bold ${
                    ...
                    <div>
                      <div className="flex justify-between text-[11px] font-medium text-slate-500 mb-1">
                        ...
                        <span className="font-mono font-bold text-slate-700">{item.consumption_rate}%</span>
                      </div>
              ...
              {/* Récapitulatif Total Mobile */}
              <div className="p-4 bg-slate-50 border-t-2 border-slate-200 space-y-2">
                <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                  <span>Total Prévisionnel :</span>
                  <span className="font-mono text-slate-900">{fmt(summary.total_forecast)}</span>
                </div>
                <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                  <span>Total Réel Dépensé :</span>
                  <span className="font-mono text-indigo-700 text-sm font-black">{fmt(summary.total_real)}</span>
                </div>
                <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${
                              isSolde ? 'bg-emerald-500' : isPartiel ? 'bg-amber-500' : 'bg-slate-400'
                            }`}
                          />
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            {isManager && (
              ...
              >
                <Plus className="w-4 h-4" />
                <span>Nouveau poste de dépense</span>
              </button>
            )}
          </div>
          ...
          {summary.items.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center text-slate-500 shadow-xs">
              ...
              <p className="text-base font-bold text-slate-800">
                Aucun poste de dépense pour l'année {selectedYear}
              </p>
              <p className="text-xs text-slate-500 mt-1 max-w-md mx-auto">
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <h3 className="font-extrabold text-slate-900 text-base">
                {editingItem ? 'Modifier le poste de dépense' : 'Nouveau poste de dépense'}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveItem} className="p-5 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Nom du poste de dépense *</label>
                <input
                  type="text"
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-2xs flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="font-extrabold text-slate-900 text-base">
                  Suivi des paiements : {selectedFundCallDetail.call_number}
                </h3>
